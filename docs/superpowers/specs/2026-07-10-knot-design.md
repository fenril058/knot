...
- ページの先頭行はタイトルである（Cosense と同じ）。コミット適用の結果として先頭行のテキストが変わった場合はタイトル変更として扱い、`pages.title` と `title_lc` を同時に更新する。title_lc の一意制約に反する場合はコミット全体を 409 で拒否する。
- ページの新規作成は「タイトル行 1 行だけの insert を含む最初のコミット」で行う。空のページ行は作らない。
- ページ削除も「全行の delete」のコミットとして記録する。すべての書き込みが commits に残る、という不変条件はここでも保つ。
...
- タイトルが既存ページと衝突した場合の動作は、インポート時のオプションで「スキップ」「上書き」「更新日時（updated）が新しい方を残す」から選ぶ。既定はスキップ。
