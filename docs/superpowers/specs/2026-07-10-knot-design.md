...
- URL のペーストをリンク記法に変換する。テキストを選択して URL をペーストすると `[URL 選択テキスト]` になる。ただし、書き手が URL を生のまま置こうとしていると解釈できる文脈では発動しない。コードブロック内、インラインコード内、既存のブラケット `[ ]` の中がその例である。この判定は文脈述語（カーソル位置のパース文脈を受け取り、変換の可否を返す関数）のリストとして実装し、除外条件を後から追加できるようにする
