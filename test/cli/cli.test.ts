...
import { CliError, runAccountAdd, runExport, runImport, runInit, runReindex } from '../../src/cli/commands.ts';
...
void test('account add が Account と Actor を作り、同名の再実行は失敗する', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'knot-cli-'));
  await runInit(dir);
  const out = await runAccountAdd(dir, 'alice', 'Alice', true, 'pw12345678');
  assert.match(out, /alice/);
  await assert.rejects(runAccountAdd(dir, 'alice', null, false, 'other-pass'), /already exists/);
});

void test('account add は短いパスワードと不正な名前を拒否する', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'knot-cli-'));
  await runInit(dir);
  await assert.rejects(runAccountAdd(dir, 'alice', null, false, 'short'), CliError);
  await assert.rejects(runAccountAdd(dir, 'Bad Name!', null, false, 'pw12345678'), CliError);
});
