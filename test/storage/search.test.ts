...
void test('3 文字以上は FTS で全文検索し、一致行を返す', async () => {
  ...
  const hits = await storage.search(project.id, parseSearchQuery('設計書'));
...
void test('3 文字未満は LIKE フォールバックで見つかる', async () => {
  ...
  const hits = await storage.search(project.id, parseSearchQuery('設計'));
  assert.equal(hits.length, 1);
  assert.equal(hits[0]!.pageId, 'pg1');
...
void test('複数の短い検索語にも AND と除外を適用する', async () => {
  const { storage, project } = await setup();
  const andHits = await storage.search(project.id, { words: ['設計', '検索'], excludes: [] });
  assert.deepEqual(andHits.map((hit) => hit.pageId), ['pg1']);
  assert.deepEqual(andHits[0]!.lines, ['knot 設計書', '検索の設計を書く']);

  const excluded = await storage.search(project.id, { words: ['設計'], excludes: ['検索'] });
...
void test('LIKE のメタ文字はリテラルとして扱う', async () => {
  ...
  const percent = await storage.search(project.id, parseSearchQuery('100%達成'));
...
void test('二重引用符を含むクエリでもクラッシュしない', async () => {
  const { storage, project } = await setup();
  const hits = await storage.search(project.id, parseSearchQuery('ab"cd'));
...
void test('空クエリは 0 件、削除済みページはヒットしない', async () => {
  const { storage, project } = await setup();
  assert.deepEqual(await storage.search(project.id, parseSearchQuery('')), []);
...
void test('SearchHit に image が含まれる', async () => {
  ...
  const hits = await storage.search(project.id, parseSearchQuery('needle'));
